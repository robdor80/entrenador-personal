...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("guardar-entreno")
    .addEventListener("click", async () => {
      const tipo = document.querySelector(".entreno-btn.seleccionado")?.dataset
        .tipo;
  ...
  document
    .getElementById("cancelar-entreno")
...
function mostrarCalendario() {
  ...
  for (let dia = 1; dia <= diasMes; dia++) {
    ...
    const tipoTurno =
      cicloTurnos[((diffDias % cicloTurnos.length) + cicloTurnos.length) % cicloTurnos.length];
    ...
    html += `
      <div class="dia" style="border-left: 6px solid ${color}; ${estiloHoy}" data-dia="${dia}" title="Turno: ${tipoTurno}">
        ...
        ${
          entrenoDelDia
            ? `<span class="entreno-asignado">${entrenoDelDia}</span>`
            : ""
        }
      </div>`;
  }
...
async function cargarEntrenosDelMes() {
  ...
  if (snap.exists()) {
    entrenosGuardados = snap.data();
  } else {
    entrenosGuardados = {};
  }
}

// Marcar botón entreno seleccionado
